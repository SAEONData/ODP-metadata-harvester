...
class ISO19115chemaGenerator(Schema):
    ...
    def set_DOI(self, DOI):
        if not DOI:
            logger.warning(f'Record:{self.record_id}- Mandatory field: DOI is empty')
            return
        self.record['doi'] = DOI.strip()

    def set_title(self, title):
        if not title:
            logger.warning(f'Record:{self.record_id}- Mandatory field: Title is empty')
            return
        self.record["title"] = title.strip()

    def set_date(self, date):
        if not date:
            logger.warning(f'Record:{self.record_id}- Mandatory field: date is empty')
            return
    ...
    def set_responsible_party(self, responsible_parties_array):
        if not responsible_parties_array:
            logger.warning(f'Record:{self.record_id}- Mandatory field: Responsible Parties is empty')
            return
    ...
    def set_geographic_identifier(self, identifier):
        if not identifier:
            logger.warning(f'Record:{self.record_id}: Mandatory Geographic Identifier field is empty')
            return
        else:
    ...
    def add_bounding_polygon(self, polygon):
        if not polygon:
            logger.warning(f'{self.record_id}: Bounding polygon is empty, continuing with record')
            return
    ...
    def set_vertical_extent(self, vertical_elements):
        if not vertical_elements:
            logger.warning(f'{self.record_id}: Vertical elements is empty, continuing with record')
            return
        vertical_extent = {
    ...
    def set_temporal_extent(self, start_time, end_time):
        if not start_time or not end_time:
            logger.warning(f'Record:{self.record_id}: Mandatory start_time or end_time field is empty')
            return
    ...
    def set_languages(self, language):
        if not language:
            logger.warning(f'Record:{self.record_id}: Mandatory language field is empty')
            return
        self.record["languages"] = [language.strip()]

    def set_characterset(self, characterset):
        if not characterset:
            logger.warning(f'Record:{self.record_id}: Mandatory character set field is empty')
            return
        self.record["characterSet"] = characterset

    def set_topic_categories(self, categories):
        if not categories:
            logger.warning(f'Record:{self.record_id}: Mandatory topic categories set field is empty')
            return
    ...
    def set_spatial_resolution(self, resolution):
        if not resolution:
            ...
            logger.warning(f'Record:{self.record_id}: Spatial Resolution is empty, continuing with record')
            return
        self.record["spatialResolution"] = resolution.strip()

    def set_abstract(self, abstract):
        if not abstract:
            logger.warning(f'Record:{self.record_id}: Mandatory abstract set field is empty')
            return
        self.record["abstract"] = abstract.strip()

    def set_distribution_format(self, format_name, format_version=None):
        if not format_name:
            logger.warning(f'Record:{self.record_id}: Mandatory distribution format set field is empty')
            return
    ...
    def set_spatial_representation_type(self, represenation):
        if not represenation:
            logger.warning(f'Record:{self.record_id}: Mandatory spatial resolution type field is empty')
            return
    ...
    def set_reference_system_name(self, reference):
        if not reference:
            logger.warning(f'Record:{self.record_id}: Mandatory reference system name field is empty')
            return
    ...
    def set_lineage_statement(self, lineage):
        if not lineage:
            logger.warning(f'Record:{self.record_id}: Mandatory lineage field is empty')
        self.record["lineageStatement"] = lineage

    def add_online_resources(self, onlineResources):
        if not onlineResources:
            logger.warning(f'Record:{self.record_id}: Mandatory online resource field is empty')
            return
    ...
    def set_accession_identifier(self, file_identifier):
        if not file_identifier:
            logger.warning(f'Record:{self.record_id}: Mandatory fileIdentifier field is empty')
            return
    ...
    def set_metadata_standard_name(self, metadata_standard):
        if not metadata_standard:
            logger.warning(f'Record:{self.record_id}: Mandatory metadata standard name field is empty')
            return
    ...
    def set_metadata_standard_version(self, standard_version):
        if not standard_version:
            logger.warning(f'Record:{self.record_id}: Mandatory metadata standard version field is empty')
            return
        self.record["metadataStandardVersion"] = str(standard_version).strip()

    def set_metadata_language(self, language):
        if not language:
            logger.warning(f'Record:{self.record_id}: Mandatory metadata language is empty')
            return
        self.record["metadataLanguage"] = language.strip()

    def set_metadata_characterset(self, characterset):
        if not characterset:
            logger.warning(f'Record:{self.record_id}: Mandatory metadata character set is empty')
            return
        self.record["metadataCharacterSet"] = characterset.strip()

    def set_metadata_time_stamp(self, timestamp):
        if not timestamp:
            logger.warning(f'Record:{self.record_id}: Mandatory metadata time stamp is empty')
    ...
    def set_purpose(self, purpose):
        if not purpose:
            self.record["purpose"] = ''
            logger.warning(f'{self.record_id}: Purpose is empty, continuing with record')
            return
    ...
    def set_status(self, status):
        if not status:
            logger.warning(f'Record:{self.record_id}: Mandatory status is empty')
        if type(status) != list:
    ...
    def set_keywords(self, keywords):
        if not keywords:
            logger.warning(f'{self.record_id}: Keywords is empty, continuing with record')
            return
    ...
    def set_descriptive_keywords(self, keywordArray):
        if not keywordArray:
            logger.warning(f'{self.record_id}: Descriptive Keywords is empty, continuing with record')
            return
        else:
    ...
    def set_constraints(self, rights, rights_uri, access_constraints, use_constraints='', classification='',
                        use_limitations=''):
        if not rights:
            logger.warning(f'Record:{self.record_id}: Mandatory Constraints is empty')
    ...
    def set_related_identifiers(self, related_identifiers_array):
        if not related_identifiers_array:
            logger.warning(f'{self.record_id}: Related Identifiers is empty, continuing with record')
            return
