...
class ISO19115chemaGenerator(Schema):
    ...
    def _monitoring_helper(self, error=None, warning=None):
        if warning:
            logger.warning(warning)
            HarvestController.HARVEST_STATS[self.record_id]['schema_errors'].append(warning)
        else:
            logger.warning(error)
            HarvestController.HARVEST_STATS[self.record_id]['schema_errors'].append(error)

    def set_DOI(self, DOI):
        if not DOI:
            self._monitoring_helper(warning=f'Record:{self.record_id}- Mandatory field: DOI is empty')
            return
        self.record['doi'] = DOI.strip()

    def set_title(self, title):
        if not title:
            self._monitoring_helper(warning=f'Record:{self.record_id}- Mandatory field: Title is empty')
            return
        self.record["title"] = title.strip()

    def set_date(self, date):
        if not date:
            self._monitoring_helper(warning=f'Record:{self.record_id}- Mandatory field: date is empty')
            return
    ...
    def set_responsible_party(self, responsible_parties_array):
        if not responsible_parties_array:
            self._monitoring_helper(warning=f'Record:{self.record_id}- Mandatory field: Responsible Parties is empty')
            return
    ...
    def set_geographic_identifier(self, identifier):
        if not identifier:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory Geographic Identifier field is empty')
            return
        else:
    ...
    def set_bounding_box_extent(self, bounding_box):
        if not bounding_box:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory Bounding Box field is empty')
            return
        ...
        try:
            ...
            self._monitoring_helper(warning=f'Record:{self.record_id}: Bounding box contains non float/int value {e}')

    def add_bounding_polygon(self, polygon):
        if not polygon:
            self._monitoring_helper(warning=f'{self.record_id}: Bounding polygon is empty, continuing with record')
            return
    ...
    def set_vertical_extent(self, vertical_elements):
        if not vertical_elements:
            self._monitoring_helper(warning=f'{self.record_id}: Vertical elements is empty, continuing with record')
            return
        vertical_extent = {
    ...
    def set_temporal_extent(self, start_time, end_time):
        if not start_time or not end_time:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory start_time or end_time field is empty')
            return
    ...
    def set_languages(self, language):
        if not language:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory language field is empty')
            return
        self.record["languages"] = [language.strip()]

    def set_characterset(self, characterset):
        if not characterset:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory character set field is empty')
            return
        self.record["characterSet"] = characterset

    def set_topic_categories(self, categories):
        if not categories:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory topic categories set field is empty')
            return
    ...
    def set_spatial_resolution(self, resolution):
        if not resolution:
            ...
            self._monitoring_helper(warning=f'Record:{self.record_id}: Spatial Resolution is empty, continuing with record')
            return
        self.record["spatialResolution"] = resolution.strip()

    def set_abstract(self, abstract):
        if not abstract:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory abstract set field is empty')
            return
        self.record["abstract"] = abstract.strip()

    def set_distribution_format(self, format_name, format_version=None):
        if not format_name:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory distribution format set field is empty')
            return
    ...
    def set_spatial_representation_type(self, represenation):
        if not represenation:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory spatial resolution type field is empty')
            return
    ...
    def set_reference_system_name(self, reference):
        if not reference:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory reference system name field is empty')
            return
    ...
    def set_lineage_statement(self, lineage):
        if not lineage:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory lineage field is empty')
    ...
    def set_metadata_standard_name(self, metadata_standard):
        if not metadata_standard:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory metadata standard name field is empty')
            return
    ...
    def set_metadata_standard_version(self, standard_version):
        if not standard_version:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory metadata standard version field is empty')
            return
        self.record["metadataStandardVersion"] = str(standard_version).strip()

    def set_metadata_language(self, language):
        if not language:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory metadata language is empty')
            return
        self.record["metadataLanguage"] = language.strip()

    def set_metadata_characterset(self, characterset):
        if not characterset:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory metadata character set is empty')
            return
        self.record["metadataCharacterSet"] = characterset.strip()

    def set_metadata_time_stamp(self, timestamp):
        if not timestamp:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory metadata time stamp is empty')
    ...
    def set_purpose(self, purpose):
        if not purpose:
            self.record["purpose"] = ''
            self._monitoring_helper(warning=f'{self.record_id}: Purpose is empty, continuing with record')
            return
        self.record["purpose"] = purpose

    def set_scope(self, scope):
        if not scope:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory scope is empty')
            return
        self.record["scope"] = scope.strip()

    def set_status(self, status):
        if not status:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory status is empty')
        if type(status) != list:
    ...
    def set_keywords(self, keywords):
        if not keywords:
            self._monitoring_helper(warning=f'{self.record_id}: Keywords is empty, continuing with record')
            return
    ...
    def set_descriptive_keywords(self, keywordArray):
        if not keywordArray:
            self._monitoring_helper(warning=f'{self.record_id}: Descriptive Keywords is empty, continuing with record')
            return
        else:
    ...
    def set_constraints(self, rights, rights_uri, access_constraints, use_constraints='', classification='',
                        use_limitations=''):
        if not rights:
            self._monitoring_helper(warning=f'Record:{self.record_id}: Mandatory Constraints is empty')
    ...
    def set_related_identifiers(self, related_identifiers_array):
        if not related_identifiers_array:
            self._monitoring_helper(warning=f'{self.record_id}: Related Identifiers is empty, continuing with record')
            return
